...
`/api/health` is open. `/api/sap-webhook` takes no bearer token but requires its
own shared-secret header instead, because SAP is a machine caller (see below).
...
with no way to log in -- but **not open either**: every request must carry a
matching `X-SAP-Secret` header or it is rejected with `401`.

An inbound webhook is an unauthenticated write path into the database, so
leaving it open would let anyone who knows the URL create inspection records.
The demo secret is `sap-dev-secret`; override it with `SAP_WEBHOOK_SECRET`.
